...
export default function Settings () {
    const settings = useSelector(state => state.main.settings.config);
    const settingsStatus = useSelector(state => state.main.settings.status);
    const dispatch = useDispatch();

    const setConfigValue = (name) => {
      return function(value) {
        let id = settings.id;
        let payload = {[name]: value};
        dispatch(setSettings({id, payload}));
      };
    }
    ...
    const notificationSettings = [
      ...
      },
      {
        ...
        type: 'timedelta',
        value: settings.notify_on_expiration_before.split(" ")[0],
