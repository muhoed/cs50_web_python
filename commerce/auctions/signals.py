...
def listing_handler(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        sure you ship the product to the winner at her/his delivery address.<br>%(address)s.\
        <br>This message is autogenerated. Do not reply.") %\
                    {
...
def comment_handler(sender, instance, **kwargs):
    ...
    on %(time).<br>Comment's content:<br>%(content)<br>Go to <a href='%(url)s'>here</a> \
...
def pre_bid_handler(sender, instance, **kwargs):
    if float(instance.value) <= float(instance.listing.max_bid):
        ...
        raise ValidationError(f"Your bid is less or equal to the current \
                            ...
                            try again. Current highest bid is %s" \
                            % str(instance.listing.max_bid))
